...
void ExpDCacheAdd(const BIGNUM *P, const BIGNUM *Q, const BIGNUM *E,
                  const BIGNUM *D)
{
    ...
    if (!dce->P || !dce->Q || !dce->E || !dce->D)
        ExpDCacheEntryFree(dce);
    else
        ExpDCacheIncrementAge(~0);
}

BIGNUM *ExpDCacheFind(const BIGNUM *P, const BIGNUM *Q, const BIGNUM *E)
{
    size_t i;
    unsigned myage;

    for (i = 0; i < ARRAY_SIZE(ExpDCache); i++) {
        if (BN_cmp(ExpDCache[i].P, P) == 0 && BN_cmp(ExpDCache[i].Q, Q) == 0 &&
